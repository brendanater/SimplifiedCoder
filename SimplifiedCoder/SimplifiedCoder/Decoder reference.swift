//
//  Decoder reference.swift
//  SimplifiedCoder
//
//  Created by Brendan Henderson on 8/30/17.
...
class _JSONDecoder2 : Decoder {
    // MARK: Properties
    /// The decoder's storage.
    fileprivate var storage: _JSONDecodingStorage
    
    typealias Options = (dateDecodingStrategy: JSONDecoder.DateDecodingStrategy, dataDecodingStrategy: JSONDecoder.DataDecodingStrategy, nonConformingFloatDecodingStrategy: JSONDecoder.NonConformingFloatDecodingStrategy, userInfo: [CodingUserInfoKey : Any])
    ...
    init(referencing container: Any, at codingPath: [CodingKey] = [], options: Options) {
    ...
    public func container<Key>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> {
        ...
        let container = _JSONKeyedDecodingContainer2<Key>(referencing: self, wrapping: topContainer)
    ...
    public func unkeyedContainer() throws -> UnkeyedDecodingContainer {
        ...
        return _JSONUnkeyedDecodingContainer2(referencing: self, wrapping: topContainer)
    }
...
struct _JSONKeyedDecodingContainer2<K : CodingKey> : KeyedDecodingContainerProtocol {
    typealias Key = K
    
    // MARK: Properties
    /// A reference to the decoder we're reading from.
    private let decoder: _JSONDecoder2
    ...
    fileprivate init(referencing decoder: _JSONDecoder2, wrapping container: [String : Any]) {
    ...
    public func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type, forKey key: Key) throws -> KeyedDecodingContainer<NestedKey> {
        ...
        let container = _JSONKeyedDecodingContainer2<NestedKey>(referencing: self.decoder, wrapping: dictionary)
        return KeyedDecodingContainer(container)
    }
    ...
    public func nestedUnkeyedContainer(forKey key: Key) throws -> UnkeyedDecodingContainer {
        ...
        return _JSONUnkeyedDecodingContainer2(referencing: self.decoder, wrapping: array)
    }
    ...
    private func _superDecoder(forKey key: CodingKey) throws -> Decoder {
        ...
        return _JSONDecoder2(referencing: value, at: self.decoder.codingPath, options: self.decoder.options)
    }
...
struct _JSONUnkeyedDecodingContainer2 : UnkeyedDecodingContainer {
    // MARK: Properties
    /// A reference to the decoder we're reading from.
    private let decoder: _JSONDecoder2
    ...
    // MARK: - Initialization
    /// Initializes `self` by referencing the given decoder and container.
    fileprivate init(referencing decoder: _JSONDecoder2, wrapping container: [Any]) {
        self.decoder = decoder
        self.container = container
        self.codingPath = decoder.codingPath
        self.currentIndex = 0
...
extension _JSONDecoder2 : SingleValueDecodingContainer {
